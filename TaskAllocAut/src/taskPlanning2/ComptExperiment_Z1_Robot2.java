...
public class ComptExperiment_Z1_Robot2 {
...
//		tsGraph.addEdge(nodet, nodep, new TSEdge("p"));
	...
	public static void main(String args[]) throws ExportException, IOException {
		...
		Robot robot0 = new Robot(buildRobotActionList(0), buildTsGraph(0), initNode1);
		
		ArrayList<String> initNode2 = new ArrayList<String>(Arrays.asList("h"));
		Robot robot1 = new Robot(buildRobotActionList(1), buildTsGraph(1), initNode2);
		Set<Robot> providedRobots1 = new HashSet<>(Arrays.asList(robot0));
		Set<Robot> providedRobots2 = new HashSet<>(Arrays.asList(robot1));
		Set<Robot> providedRobots = new HashSet<>(Arrays.asList(robot0, robot1));
...
//			add('g');
		...
		for (int index = 0; index < decompositionSet.size(); index++) {
			...
			robotSetSwitchManned.autRobotsPair.add(
					new RobotConfig(new TaskAutomaton(decompositionSet.get(index).get(0)), providedRobots1)
					);
			robotSetSwitchManned.autRobotsPair.add(
					new RobotConfig(new TaskAutomaton(decompositionSet.get(index).get(1)), providedRobots2)
					);
